...
internal readonly record struct RegisterDef {
    ...
    public RegisterDef(string enumMemberName, string name, bool hasNumber, int number, int size, bool isGpr) {
...
public class RegisterInfoGenerator : IIncrementalGenerator{
    ...
    private const string RegisterAttributeText = 
        """
        // <auto-generated />
        namespace SAAE.Generators {
    ...
    private const string BankDefinitionAttributeText = 
        """
        // <auto-generated />
        namespace SAAE.Generators {
        
            [System.AttributeUsage(System.AttributeTargets.Enum)]
            public class RegisterBankDefinitionAttribute : System.Attribute {
                
                public int Processor { get; init; } = 0;
            
                public RegisterBankDefinitionAttribute(SAAE.Engine.Common.Architecture architecture) {
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        context.RegisterPostInitializationOutput(ctx => {
            ctx.AddSource("RegisterBankDefinitionAttribute.g.cs", SourceText.From(BankDefinitionAttributeText, Encoding.UTF8));
            ctx.AddSource("RegisterAttribute.g.cs", SourceText.From(RegisterAttributeText, Encoding.UTF8));
        });
    ...
    private static EnumToGenerate GetSemanticTargetForGeneration(GeneratorAttributeSyntaxContext ctx) {
        ...
        for (int i = 0; i < eds.Members.Count; i++) {
            ...
            foreach (AttributeData attribute in attributes) {
                if (attribute.AttributeClass!.ToDisplayString() != "SAAE.Generators.RegisterAttribute") continue;
                if (attribute.ConstructorArguments.Length == 4) {
                    regs.Add(new RegisterDef(fieldSymbol.ToDisplayString(), (string)attribute.ConstructorArguments[1].Value!, true, (int)attribute.ConstructorArguments[0].Value!, (int)attribute.ConstructorArguments[2].Value!, (bool)attribute.ConstructorArguments[3].Value!));
                }
                else {
                    regs.Add(new RegisterDef(fieldSymbol.ToDisplayString(), (string)attribute.ConstructorArguments[0].Value!, false, -1, (int)attribute.ConstructorArguments[1].Value!, (bool)attribute.ConstructorArguments[2].Value!));
                }
                break;
            }
        }
