using Mercury.Engine.Common;
using Mercury.Engine.Generators.Instruction;

namespace Mercury.Engine.Mips.Instructions;

[Instruction]
[FormatExact(31,26,17)]
[FormatExact(25,21,[16,17,20])]
[FormatExact(5,0,1)]
public partial class SubFloat : IInstruction {

    [Field(25,21)]
    public byte Format { get; set; }

    [Field(20,16)]
    public byte Ft { get; set; }

    [Field(15,11)]
    public byte Fs { get; set; }

    [Field(10,6)]
    public byte Fd { get; set; }

    public bool IsDouble => Format == Instruction.DoublePrecisionFormat;

    public override string ToString() => $"sub.{Instruction.FpuFormatFmt(Format)} ${Instruction.FpuTranslateRegisterName(Fd)}, ${Instruction.FpuTranslateRegisterName(Fs)}, ${Instruction.FpuTranslateRegisterName(Ft)}";
}